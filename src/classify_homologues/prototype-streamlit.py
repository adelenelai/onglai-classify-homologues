...
st.markdown('Homologous series are groups of related chemical compounds that share a common core substructure and a repeating chemical subunit. Compounds in a homologous series can be represented by a general formula or Markush structure. A basic example is alkanes (CnH2n+1): methane, ethane, propane *etc.*')
st.markdown('Many areas of chemistry have homologous series, such as natural products and environmental chemicals (*e.g.,* High Production Volume chemicals like surfactants). Because of their structural similartiy, compounds within the same homologous series typically share similar or predictable properties, and may produce distinct analytical signatures.')

st.subheader('Classifiying Homologous Series')
st.markdown('Within most compound datasets or databases, homologous series are typically unclassified. Therefore, an algorithm was developed and implemented using the RDKit to classify homologous series.')
...
df = pd.read_csv('classified_series.csv') #default test1_23.csv as trial data within Docker image?
...
## Number input
series_select = st.number_input(label='Select a classified homologous series by series_no:',
                                min_value=-3,
...
raw_html_series = mols2grid.display(df_result,
...
st.markdown('1. ```Lai, A., Schaub, J., Steinbeck, C., Schymanski, E. L. (2022). An Algorithm to Classify Homologous Series.```*in prep*')
st.markdown('2. [GitHub Repository](https://github.com/adelenelai/classify_homologues) containing HS classification algorithm code.')
